...
def upgrade():
    op.execute("""
        ...
        UPDATE OF
        id_dataset, observers_txt, id_digitiser, grp_method, id_nomenclature_grp_typ, date_min, hour_min, date_max, hour_max, altitude_min, altitude_max, depth_min, depth_max, place_name, precision, geom_local, geom_4326, id_nomenclature_geo_object_nature, comment, additional_fields
        ON
        pr_occtax.t_releves_occtax FOR EACH ROW EXECUTE FUNCTION pr_occtax.fct_tri_synthese_update_releve();
        ...
        OF geom_4326
            ON
            pr_occtax.t_releves_occtax FOR EACH ROW EXECUTE FUNCTION ref_geo.fct_trg_calculate_geom_local('geom_4326',
            'geom_local');
    """)
